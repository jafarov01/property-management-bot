...
async def get_property_from_context(update: Update, context_args: list, db: Session):
    ...
    if not context_args:
        ...
        error_message = telegram_client.format_simple_error(f"Property code is required.\n{usage_example}")
    ...
    if not prop:
        error_message = telegram_client.format_simple_error(f"Property `{prop_code}` not found in the database.")
        await update.message.reply_text(error_message, parse_mode='Markdown')
        return None
