...
# VERSION: 6.4 (Async Database Refactor)
# ==============================================================================
...
async def parse_email_in_background(alert_id: int, email_uid: str, *, db: AsyncSession):
    ...
    try:
        ...
        if not email_body:
            ...
            stmt = update(models.EmailAlert).where(models.EmailAlert.id == alert_id).values(
        ...
        stmt = select(models.EmailAlert).filter(models.EmailAlert.id == alert_id)
...
async def unhandled_issue_reminder_task(*, db: AsyncSession):
    ...
    stmt = select(models.EmailAlert).filter(
...
async def daily_briefing_task(time_of_day: str, *, db: AsyncSession):
    ...
    occupied_stmt = select(func.count(models.Property.id)).filter(models.Property.status == models.PropertyStatus.OCCUPIED)
    pending_stmt = select(func.count(models.Property.id)).filter(models.Property.status == models.PropertyStatus.PENDING_CLEANING)
    maintenance_stmt = select(func.count(models.Property.id)).filter(models.Property.status == models.PropertyStatus.MAINTENANCE)
    available_stmt = select(func.count(models.Property.id)).filter(models.Property.status == models.PropertyStatus.AVAILABLE)
    
    occupied_result = await db.execute(occupied_stmt)
    pending_result = await db.execute(pending_stmt)
    maintenance_result = await db.execute(maintenance_stmt)
    available_result = await db.execute(available_stmt)
    
    occupied = occupied_result.scalar()
    pending = pending_result.scalar()
    maintenance = maintenance_result.scalar()
...
async def daily_midnight_task(*, db: AsyncSession):
    ...
    bot = Bot(token=config.TELEGRAM_BOT_TOKEN)
    try:
        stmt = select(models.Property).filter(models.Property.status == models.PropertyStatus.PENDING_CLEANING)
        result = await db.execute(stmt)
        ...
        # Update all properties to AVAILABLE status
        update_stmt = update(models.Property).where(
            models.Property.status == models.PropertyStatus.PENDING_CLEANING
